...
/* End of code added by Graham Mitchell. */
  ...
  /* What we'd ultimately like is to be able to accept either filenames/urls
     on the command-line, playlists, or both.  For shuffle to work properly,
	 we need a big list with all files/urls specified on the command line and
	 the contents of any playlists.

	 When the last file has been played, the player should just sleep until
	 the monitored playlist changes.

	 If in daemon mode, this should be the default, or maybe there should
	 an option for this (keep running when no files remaining).

	 Certainly in foreground mode, the player should just quit when there
	 are no files left, unless specified otherwise.

	 Also, checking any number of playlists for changes could take a while.
	 Maybe have the program that writes the playlist (or somebody) send
	 SIGHUP when the file should be re-read, and ignore them otherwise.

	 Then the signal handler would have to be modified to re-read on SIGHUP
	 if daemonizes or quit otherwise.

	 In any case, handling mixed files with playlists should be pretty easy
	 to add.  It's the "watch for changes" part that's tricky.
