...
#include <unistd.h>
#include <sys/time.h>
#include <sys/stat.h>  /* added by Graham Mitchell */
...
/* Graham Mitchell added this global variable. */
...
void daemonize()
{
  ...
  if ( pid > 0 ) {
    // parent process; write child's pid to /var/run/ogg123d.pid
    fp = fopen( "/var/run/ogg123d.pid","w" );
    if ( ! fp )
      fprintf(stderr, "ogg123d: unable to write to /var/run/ogg123d.pid\n");
	else {
	  fprintf(fp, "%d\n", pid);
      fclose(fp);
	}
    // then close the parent process
    exit(0);
  }
  else if ( pid < 0 ) {
    fprintf(stderr, "ogg123d: unable to fork()\n");
    exit(1);
  }
}
...
int make_playlist( int argc, char** argv )
{
  ...
  /* This argc and argv don't include the command name, so we start at 0 */
  for ( i=0 ; i < argc ; i++ ) {
    if ( is_ogg(argv[i]) ) {
...
int polled_file_changed()
{
  ...
  stat(options.poll_file, &statbuf);
...
/* End of functions added by Graham Mitchell. */
...
{
  int optind;
/* Graham Mitchell added these variables. */
  int current_track;
  int tracks;
/* End of code added by Graham Mitchell. */
  ...
  if (optind == argc && ! options.poll_file) {
    cmdline_usage();
    exit(1);
  }
...
/* Graham Mitchell added this code. */
  ...
  if ( options.poll_file ) {
    /* make_playlist expects argc/argv parameters, so we fake them */
    tracks = make_playlist( 1, &options.poll_file );
	polled_file_changed(); /* sets up initial checksum */
  } else {
    tracks = make_playlist( argc-optind, &argv[optind] );
  }
...
/* Graham Mitchell modified this code. */
  ...
  /* Play the files/streams from the playlist */

  current_track = 0;
  while ( !sig_request.exit ) {

	while (current_track < tracks && !sig_request.exit) {
	  play(playlist[current_track]);
	  current_track++;
	  if ( options.poll_file && polled_file_changed() ) {
		tracks = make_playlist( 1, &options.poll_file );
		current_track = 0;
	  }
	}
	if ( options.status_file && current_track == tracks ) {
	  output_now_playing( NULL );
	  current_track++;
	}
	if ( options.poll_file ) {
	  if ( ! polled_file_changed() )
		sleep(10);
	  else {
		tracks = make_playlist( 1, &options.poll_file );
		current_track = 0;
