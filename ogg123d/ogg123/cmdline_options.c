...
/* Graham Mitchell added this code. */
    /* ogg123d-custom options */
    {"daemonize", no_argument, 0, 'D'},
    {"status-file", required_argument, 0, 'S'},
...
/* Graham Mitchell added a "D","S:" and "P:" to this line. */
  while (-1 != (ret = getopt_long(argc, argv, "b:c::d:Df:hl:k:o:p:P:qS:vVx:y:z",
...
/* Graham Mitchell added this code. */
      case 'D':
	ogg123_opts->daemonize = 1;
	ogg123_opts->verbosity = -1;
	break;

	  case 'S':
	ogg123_opts->status_file = strdup(optarg);  
	break;

	  case 'P':
	ogg123_opts->poll_file = strdup(optarg);  
	break;
/* End of code added by Graham Mitchell. */
...
void cmdline_usage (void)
{
  ...
  printf (
/* Graham Mitchell added this code. */
	 "  -D, --daemonize   run player in the background.  Also sets -q.\n"
	 "  -S, --status-file=filename   Write track names to this file as\n"
	 "      they are played.\n"
	 "  -P, --poll=filename   Re-read file for changes after playing every\n"
	 "      track.  Program doesn't quit when playlist is empty.  All other\n"
	 "      tracks/playlists specified on the command-line are ignored.\n"
/* End of code added by Graham Mitchell. */
